...
class TestCaseWithDisposition(unittest.TestCase):
    ...
    def test_it_parses_all_charge_rows(self):
        assert len(self.parser._charge_table_data) == 15

    def test_ids_are_collected(self):
        assert self.parser._charge_table_data[0] == '1.\n            \xa0'
        assert self.parser._charge_table_data[5] == '2.\n            \xa0'
        assert self.parser._charge_table_data[10] == '3.\n            \xa0'

    def test_charge_names_are_collected(self):
        assert self.parser._charge_table_data[1] == 'Driving Uninsured'
        assert self.parser._charge_table_data[6] == 'Violation Driving While Suspended or Revoked'
        assert self.parser._charge_table_data[11] == 'Failure to Obey Traffic Control Device'

    def test_statutes_are_collected(self):
        assert self.parser._charge_table_data[2] == '806.010'
        assert self.parser._charge_table_data[7] == '811.175'
        assert self.parser._charge_table_data[12] == '811.265'

    def test_charge_levels_are_collected(self):
        assert self.parser._charge_table_data[3] == 'Violation Class B'
        assert self.parser._charge_table_data[8] == 'Violation Class A'
        assert self.parser._charge_table_data[13] == 'Violation Class C'

    def test_charge_dates_are_collected(self):
        assert self.parser._charge_table_data[4] == '03/12/2017'
        assert self.parser._charge_table_data[9] == '04/01/2016'
        assert self.parser._charge_table_data[14] == '05/22/2015'

    # Tests disposition data is collected from the events table
    def test_it_parses_every_row_of_the_events_table(self):
        assert len(self.parser._event_table_data) == 14

    def test_it_collects_the_disposition_row(self):
        assert self.parser._event_table_data[1][0] == '06/12/2017'
        assert self.parser._event_table_data[1][3] == 'Disposition'
        assert self.parser._event_table_data[1][4] == ' (Judicial Officer: Office, Judicial M)'
        assert self.parser._event_table_data[1][5] == '1.\xa0Driving Uninsured'
        assert self.parser._event_table_data[1][6] == 'Convicted - Failure to Appear'
        assert self.parser._event_table_data[1][7] == '2. Violation Driving While Suspended or Revoked'
        assert self.parser._event_table_data[1][8] == 'Dismissed'
        assert self.parser._event_table_data[1][9] == '3. Failure to Obey Traffic Control Device'
        assert self.parser._event_table_data[1][10] == 'Hmmmm'
        assert self.parser._event_table_data[1][11] == '\n        Created: 06/12/2017 2:57 PM'
...
class TestCaseWithoutFinancialTable(unittest.TestCase):
    ...
    # and is appended to the charge_table_data list grouped by rows.
    def test_it_parses_all_charge_rows(self):
        assert len(self.parser._charge_table_data) == 5

    def test_ids_are_collected(self):
        assert self.parser._charge_table_data[0] == '1.\n            \xa0'

    def test_charge_names_are_collected(self):
        assert self.parser._charge_table_data[1] == 'Poss Controlled Sub 2'

    def test_statutes_are_collected(self):
        assert self.parser._charge_table_data[2] == '4759924B'

    def test_charge_levels_are_collected(self):
        assert self.parser._charge_table_data[3] == 'Felony Class C'

    def test_charge_dates_are_collected(self):
        assert self.parser._charge_table_data[4] == '04/12/1992'
    ...
    def test_it_collects_the_disposition_row(self):
        assert self.parser._event_table_data[0][0] == '04/30/1992'
        assert self.parser._event_table_data[0][3] == 'Disposition'
        assert self.parser._event_table_data[0][4] == ' (Judicial Officer: Unassigned, Judge)'
        assert self.parser._event_table_data[0][5] == '1. Poss Controlled Sub 2'
        assert self.parser._event_table_data[0][6] == 'Dismissed'
        assert self.parser._event_table_data[0][7] == '\n        Created: 05/01/1992 12:00 AM'
...
class TestCaseWithPartialDisposition(unittest.TestCase):
    ...
    # and is appended to the charge_table_data list grouped by rows.
    def test_it_parses_all_charge_rows(self):
        assert len(self.parser._charge_table_data) == 20

    def test_ids_are_collected(self):
        assert self.parser._charge_table_data[0] == '1.\n            \xa0'
        assert self.parser._charge_table_data[5] == '2.\n            \xa0'
        assert self.parser._charge_table_data[10] == '3.\n            \xa0'
        assert self.parser._charge_table_data[15] == '999.\n            \xa0'

    def test_charge_names_are_collected(self):
        assert self.parser._charge_table_data[1] == 'Reckless Driving'
        assert self.parser._charge_table_data[6] == 'Resisting Arrest'
        assert self.parser._charge_table_data[11] == 'Interfering w/ Peace/Parole and Probation Officer'
        assert self.parser._charge_table_data[16] == 'Driving Under the Influence of Intoxicants'

    def test_statutes_are_collected(self):
        assert self.parser._charge_table_data[2] == '811.140'
        assert self.parser._charge_table_data[7] == '162.315'
        assert self.parser._charge_table_data[12] == '162.247'
        assert self.parser._charge_table_data[17] == '813.010(4)'

    def test_charge_levels_are_collected(self):
        assert self.parser._charge_table_data[3] == 'Misdemeanor Class A'
        assert self.parser._charge_table_data[8] == 'Misdemeanor Class A'
        assert self.parser._charge_table_data[13] == 'Misdemeanor Class A'
        assert self.parser._charge_table_data[18] == 'Misdemeanor Class A'

    def test_charge_dates_are_collected(self):
        assert self.parser._charge_table_data[4] == '03/06/2018'
        assert self.parser._charge_table_data[9] == '03/06/2018'
        assert self.parser._charge_table_data[14] == '03/06/2018'
        assert self.parser._charge_table_data[19] == '03/06/2018'

    # Tests disposition data is collected from the events table
    def test_it_parses_every_row_of_the_events_table(self):
        assert len(self.parser._event_table_data) == 20

    def test_it_collects_the_disposition_row(self):
        assert self.parser._event_table_data[0][0] == '03/06/2018'
        assert self.parser._event_table_data[0][3] == 'Disposition'
        assert self.parser._event_table_data[0][4] == '999.\xa0Driving Under the Influence of Intoxicants'
        assert self.parser._event_table_data[0][5] == 'No Complaint'
        assert self.parser._event_table_data[0][6] == '\n        Created: 03/06/2018 1:07 PM'
...
class TestCaseWithoutDisposition(unittest.TestCase):
    ...
    # Tests disposition data is collected from the events table
    def test_it_parses_every_row_of_the_events_table(self):
        assert len(self.parser._event_table_data) == 19

    def test_it_collects_the_disposition_row(self):
        dispo_count = 0
        for data in self.parser._event_table_data:
...
class TestParkingViolationCase(unittest.TestCase):
    ...
    def test_charge_names_are_collected(self):
        assert self.parser._charge_table_data[1] == 'No Meter Receipt'

    def test_statutes_are_collected(self):
        assert self.parser._charge_table_data[2] == '16.20.430-A'

    def test_charge_levels_are_collected(self):
        assert self.parser._charge_table_data[3] == 'Violation Unclassified'

    def test_charge_dates_are_collected(self):
        assert self.parser._charge_table_data[4] == '12/01/2018'

    # Tests disposition data is collected from the events table
    def test_it_parses_every_row_of_the_events_table(self):
        assert len(self.parser._event_table_data) == 1
...
class TestCaseWithRelatedCases(unittest.TestCase):
    ...
    def test_charge_dates_are_collected(self):
        assert self.parser._charge_table_data[4] == '03/19/2000'
